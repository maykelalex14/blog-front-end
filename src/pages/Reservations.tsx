...
const Reservations: React.FC = () => {
  ...
  return (
    <Container>
      <h1 style={{color:'#e0b04b',marginBottom:24}}>Reservations</h1>
      {/* Stats Dashboard */}
      <Section>
        ...
        <Table>
          ...
          <tbody>
            {filtered.map((r: Reservation) => (
              <tr key={r.id} style={{background:r.status==='Canceled'?'#333':r.status==='Completed'?'#222':'#181818'}}>
                <Td>{r.name}</Td>
                <Td>{r.contact}</Td>
                <Td>{r.date}</Td>
                <Td>{r.time}</Td>
                ...
                <Td>
                  ...
                  <Button type="button" onClick={()=>handleDelete(r.id)} style={{background:'#b71c1c',color:'#fff'}}>Delete</Button>
                </Td>
              </tr>
            ))}
          </tbody>
