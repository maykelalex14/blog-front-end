...
const Contact: React.FC = () => {
    ...
    return (
      <Section id="contact-admin" aria-label="Contact Admin Panel">
        ...
        <div style={{marginBottom:32}}>
          <h3 style={{color:'#e0b04b',marginBottom:8}}>Customer Inquiries</h3>
          <table style={{width:'100%',background:'#181818',color:'#fff',borderRadius:8}}>
            <thead><tr style={{background:'#b71c1c'}}><th>Name</th><th>Email</th><th>Message</th><th>Date</th><th>Status</th><th>Category</th><th>Actions</th></tr></thead>
            <tbody>
              {messages.map(m=>(
          ...
          {/* Respond/Forward UI */}
          {selectedMsg && (
            <div style={{background:'#232526',padding:16,borderRadius:8,marginTop:12}}>
              <h4 style={{color:'#e0b04b'}}>Respond or Forward</h4>
        ...
        {/* Category Management */}
        <div style={{marginBottom:32}}>
          <h3 style={{color:'#e0b04b',marginBottom:8}}>Manage Categories</h3>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
            {categories.map(cat => (
              <span key={cat} style={{background:'#181818',padding:'6px 12px',borderRadius:6,display:'flex',alignItems:'center'}}>
                {cat}
                <button onClick={()=>handleDeleteCategory(cat)} style={{marginLeft:6,background:'none',color:'#b71c1c',border:'none',cursor:'pointer',fontWeight:'bold'}}>×</button>
              </span>
            ))}
          </div>
        </div>
        ...
        <div style={{marginBottom:32}}>
          <h3 style={{color:'#e0b04b',marginBottom:8}}>Branch & Support Contacts</h3>
          <table style={{width:'100%',background:'#181818',color:'#fff',borderRadius:8}}>
            <thead><tr style={{background:'#b71c1c'}}><th>Branch</th><th>Phone</th><th>Email</th><th>Actions</th></tr></thead>
            <tbody>
              {branches.map(b=>(
                <tr key={b.id}>
  ...
  return (
    <Section id="contact" aria-label="Contact">
      ...
      <ContactGrid>
        <InfoBox>
          <div><b>Phone:</b> <a href="tel:+1234567890" style={{color:'#e0b04b'}}>+1 (234) 567-890</a></div>
          <div><b>Email:</b> <a href="mailto:info@steakz.com" style={{color:'#e0b04b'}}>info@steakz.com</a></div>
          <div><b>Instagram:</b> <a href="https://instagram.com/steakzrestaurant" target="_blank" rel="noopener noreferrer" style={{color:'#e0b04b'}}>@steakzrestaurant</a></div>
        </InfoBox>
        {sent ? (
