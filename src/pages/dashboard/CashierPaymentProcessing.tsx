...
const CashierPaymentProcessing: React.FC = () => {
  ...
  const unpaidOrders = orders.filter(o => o.status !== 'fulfilled');
  // If no unpaid orders, show a message
  if (unpaidOrders.length === 0) {
    return (
      <Card>
        <h2 style={{ color: '#222', marginBottom: 24 }}>Payment Processing</h2>
        <div style={{ color: '#b71c1c', fontWeight: 600, textAlign: 'center', margin: '32px 0' }}>
  ...
  const totalAmount = order ? order.items.reduce((sum, i) => sum + (i.qty * 20), 0) : 0; // $20 per item mock
        ...
        <tbody>
          {unpaidOrders.map(o => (
            <tr key={o.id} style={{ background: selectedOrder === o.id ? '#f5f5f5' : undefined }}>
              ...
              <Td>${o.items.reduce((sum, i) => sum + (i.qty * 20), 0).toFixed(2)}</Td>
              <Td>{o.status}</Td>
